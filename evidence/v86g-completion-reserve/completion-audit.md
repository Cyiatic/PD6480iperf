...
**Subsequent feedback:** the user has now confirmed Analogue blur, menu
navigation and L-toggle behavior, and approves skipping the Hi-Res option.
See [the report](analogue-user-feedback.md) and
[completion result](completion-result.md). The audit below is the earlier
snapshot; its missing-feedback condition is superseded. Quantified performance,
endurance and physical save-import limitations are retained in the final notes.
